...
def build_transform_pipeline(dataset, aug_cfg, model_cfg) -> transforms.Compose:
    ...
    MEANS_N_STD = {
        "ben": band_combi_to_mean_std(model_cfg.data.num_bands),
        "vessel": (
            [341.45270671, 306.65436551, 301.09752831, 416.97833933],
            [742.71573792, 806.97493188, 772.28467299, 1412.17678495],
        ),
    }

    mean, std = MEANS_N_STD[model_cfg.data.dataset]
    ...
    if aug_cfg.horizontal_flip.prob:
        augmentations.append(
            transforms.RandomHorizontalFlip(p=aug_cfg.horizontal_flip.prob)
        )

    if aug_cfg.vertical_flip.prob:
        augmentations.append(
            transforms.RandomVerticalFlip(p=aug_cfg.vertical_flip.prob)
        )
