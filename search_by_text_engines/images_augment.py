...
from utils import (
    read_json,
    write_hdf5,
    get_image_file_names,
    shuffle_file_names_list,
    reshuffle_embeddings,
)
...
class ImagesDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        self, image_file_names, images_folder, img_transforms_dicts, img_aug_set
    ):
    ...
    def init_transforms(self):
        ...
        if self.img_aug_set == "each_img_random":
            ...
            return [
                self.init_transforms_not_random(
                    self.img_transforms_dicts[self.img_aug_set]
                )
            ]
    ...
    def init_transforms_random(self):
        ...
        for ts in self.img_transforms_dicts[self.img_aug_set]:
            transforms.append(
                self.init_transforms_not_random(self.img_transforms_dicts[ts])
            )
        return transforms
    ...
    def init_transforms_not_random(transform_dict):
        ...
        def _color_jittering(values):
            return torchvision.transforms.ColorJitter(
                0.8 * values, 0.8 * values, 0.8 * values, 0.2 * values
            )
...
if __name__ == "__main__":
    ...
    images_dataset = ImagesDataset(
        file_names_permutated,
        cfg.dataset_image_folder_path,
        cfg.image_aug_transform_sets,
        cfg.img_aug_set,
    )
    ...
    images_dataloader = DataLoader(
        images_dataset, batch_size=cfg.image_emb_batch_size, shuffle=False
    )
    ...
    write_hdf5(
        cfg.image_emb_aug_file, embeddings_orig_order.astype(np.float32), "image_emb"
    )
