...
parser.add_argument("--bit", default=128, help="hash code length", type=int)
...
parser.add_argument(
    "--preset",
    default="default",
    help="data presets, see available in config.py",
    type=str,
)
parser.add_argument(
    "--alpha", default=0.01, help="alpha hyperparameter (La)", type=float
)  # 0.01
parser.add_argument(
    "--beta", default=0.001, help="beta hyperparameter (Lq)", type=float
)
parser.add_argument(
    "--gamma", default=0.01, help="gamma hyperparameter (Lbb)", type=float
)
parser.add_argument(
    "--contrastive-weights",
    default=[1.0, 1.0, 1.0],
    type=float,
    nargs=3,
    metavar=("INTER", "INTRA_IMG", "INTRA_TXT"),
...
parser.add_argument(
    ...
    default=None,
    type=str,
    help="overrides augmented image embeddings file (u-curve)",
)
...
class ConfigModel(BaseConfig):
    ...
    if preset == "no_aug":
        # default for texts
        image_emb_for_model = "/data/image_emb_{}_aug_center_crop_only.h5".format(
            dataset.upper()
        )
        caption_emb_for_model = "/data/caption_emb_{}_aug.h5".format(dataset.upper())

        image_emb_aug_for_model = "/data/image_emb_{}_aug_center_crop_only.h5".format(
            dataset.upper()
        )
        caption_emb_aug_for_model = "/data/caption_emb_{}_aug.h5".format(
            dataset.upper()
        )
        ...
        image_emb_aug_for_model = "/data/image_emb_{}_aug_each_img_random.h5".format(
            dataset.upper()
        )
        caption_emb_aug_for_model = "/data/caption_emb_{}_aug_rb.h5".format(
            dataset.upper()
        )
        ...
        # default for texts
        print("default set up")
        image_emb_for_model = "/data/image_emb_{}_aug_center_crop_only.h5".format(
            dataset.upper()
        )
        caption_emb_for_model = "/data/caption_emb_{}_aug.h5".format(dataset.upper())

        image_emb_aug_for_model = "/data/image_emb_{}_aug_aug_center.h5".format(
            dataset.upper()
        )
        caption_emb_aug_for_model = "/data/caption_emb_{}_aug_rb.h5".format(
            dataset.upper()
        ...
        caption_emb_aug_for_model = "/data/caption_emb_{}_aug_bt_chain.h5".format(
            dataset.upper()
        )
        ...
        image_emb_aug_for_model = "/data/image_emb_{}_aug_aug_random.h5".format(
            dataset.upper()
        )
        caption_emb_aug_for_model = "/data/caption_emb_{}_aug_rb.h5".format(
            dataset.upper()
        ...
        image_emb_aug_for_model = "/data/image_emb_{}_aug_random_crop_only.h5".format(
            dataset.upper()
        )
        caption_emb_aug_for_model = "/data/caption_emb_{}_aug_rb.h5".format(
            dataset.upper()
        ...
        caption_emb_aug_for_model = "/data/caption_emb_{}_aug_bt_prob.h5".format(
            dataset.upper()
        )
    ...
    if dataset == "rsicd":
        label_dim = 31
    if dataset == "vessel_dataset":
        label_dim = 52

    # sto cambiando da qui; Provare ad avere un unico dataset loader
    # dataset_train_split = 0.90  # part of all data, that will be used for training
    # # (1 - dataset_train_split) - evaluation data
    # dataset_query_split = 0.10  # part of evaluation data, that will be used for query
