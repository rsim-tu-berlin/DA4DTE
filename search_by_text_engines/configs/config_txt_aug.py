...
parser.add_argument(
    "--txt-aug",
    default="backtranslation-prob",
    type=str,
    metavar="TXT_AUG_TYPE",
...
class ConfigTxtPrep(BaseConfig):
    if dataset == "ucm":
        caption_token_length = (
            64  # use hardcoded number, max token length for clean data is 26
        )

    if dataset == "rsicd":
        caption_token_length = (
            64  # use hardcoded number, max token length for clean data is 40
        )
    if dataset == "vessel_dataset":
        caption_token_length = (
            75  # use hardcoded number, max token length for clean data is 74
        )

    caption_hidden_states = 4  # Number of last BERT's hidden states to use
    caption_hidden_states_operator = (
        "sum"  # "How to combine hidden states: 'sum' or 'concat'"
    )
    ...
    caption_aug_bt_lang_weights = {
        "es": 1,
        "de": 1,
    }  # {lang1: lang_weight1, lang2: lang_weight2, ...}
    ...
    caption_emb_aug_file_bt_prob = "/data/caption_emb_{}_aug_bt_prob.h5".format(
        dataset.upper()
    )
    caption_emb_aug_file_bt_chain = "/data/caption_emb_{}_aug_bt_chain.h5".format(
        dataset.upper()
    )
