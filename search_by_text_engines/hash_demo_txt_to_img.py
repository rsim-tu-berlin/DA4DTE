...
def embed_captions(captions):
    ...
    captions_dataloader = DataLoader(
        captions_dataset, batch_size=batch_size, shuffle=False
    )
    ...
    # get BERT embeddings
    embeddings = get_embeddings_txt(
        bert, captions_dataloader, device, num_hidden_states=4, operation="sum"
    )
    return embeddings
...
def get_embeddings_img(model, img, device):
    ...
    with torch.no_grad():  # no need to call Tensor.backward(), saves memory
        ...
        output = model(
            img
        )  # (batches, batch_size, output_dim) -> (batches * batch_size, output_dim)
...
hidden_dim = 1024 * 4
hash_dim = 128
path_model = "checkpoints/vessel_dataset_" + str(hash_dim) + "_test_default"
...
def load_model(model, tag="best"):
    ...
    model.load_state_dict(
        torch.load(
            os.path.join(path_model, model.module_name + "_" + str(tag) + ".pth")
        )
    )
    x = model.load_state_dict(
        torch.load(
            os.path.join(path_model, model.module_name + "_" + str(tag) + ".pth")
        )
    )
...
captions = [
    "two vessels",
    " ",
]  # due to data loader this should be a list of at least two elemnts!
embeddings_sentence = embed_captions(captions)

dataset_image_folder_path = "./img_test/"
...
embed_torch_txt = torch.from_numpy(
    embeddings_sentence[0]
)  # [0] to get the first caption!!
...
print(
    "hash codes for the sentence: **two vessels**=",
    dict_sent_hash_zerone["two vessels"],
)
print(img)
...
print(
    "hash codes for the image filename: **T10SEG_20190808T184921_TCI_crop_x-512_y-1408.png**=",
    dict_hash_img_zerone[img],
)
# img_dictionary
