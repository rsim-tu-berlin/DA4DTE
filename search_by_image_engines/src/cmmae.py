...
class CMMAEDecoder(nn.Module):
    def __init__(
        ...
        if self.decoding_strategy == "common":
            ...
            self.decoder_pred_s1_to_s2 = nn.Linear(
                embed_dim, patch_size**2 * 10, bias=True
            )
            self.decoder_pred_s2_to_s1 = nn.Linear(
                embed_dim, patch_size**2 * 2, bias=True
            )
        else:
...
class CMMAE(pl.LightningModule):

    def __init__(
        self,
        cfg: omegaconf.DictConfig,
    ):
        """Implements CMMAE (based on MAE: https://arxiv.org/abs/2111.06377).
        ...
        self.base_model: Callable = CMMAEBackbone
    ...
    def configure_decoders(self):
        """Instantiate CMMAE decoder in a Sensor-common / -specific fashion."""
        ...
        self.decoder_s2_to_s1 = CMMAEDecoder(
            **args, decoding_strategy="specific", decoder_in_chans=2
        )
        self.decoder_s1_to_s2 = CMMAEDecoder(
    ...
    def forward(self, X: torch.Tensor) -> Dict[str, Any]:
        ...
        if self.training:
            ...
            if self.apply_cmr_loss:
                pred_s2_from_s1 = self.decode(patch_feats_s1, ids_restore_s1, "s2")
                pred_s1_from_s2 = self.decode(patch_feats_s2, ids_restore_s2, "s1")
                out.update(
                    {
    ...
    def training_step(self, batch: List[Any], batch_idx: int) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: total loss composed of CMMAE losses.
        """
        ...
        if self.apply_cmr_loss:
            ...
            metrics["train_reconstruction_loss_s1_from_s2"] = (
                reconstruction_loss_s1_from_s2
            )
            metrics["train_reconstruction_loss_s2_from_s1"] = (
                reconstruction_loss_s2_from_s1
            )
        ...
        if self.apply_umr_loss:
            ...
            metrics["train_reconstruction_loss_s1_from_s1"] = (
                reconstruction_loss_s1_from_s1
            )
            metrics["train_reconstruction_loss_s2_from_s2"] = (
                reconstruction_loss_s2_from_s2
            )
