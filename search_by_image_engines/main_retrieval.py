...
def get_features_lists(backbone, data_loader, dev):
    ...
    Args:
        backbone (torch.nn): CMMAE backbone
...
def main(ckpt_path, device):
    ...
    for aug_cfg in cfg.augmentations:
        ...
        aug_cfg.horizontal_flip.prob = 0.0
        aug_cfg.vertical_flip.prob = 0.0
        pipelines.append(
            NCropAugmentation(
    ...
    with torch.no_grad():
        ...
        archive_s1, archive_s2, archive_labels = get_features_lists(
            backbone, test_loader, dev
        )
    ...
    for from_modality, to_modality in list(itertools.product(modalities, modalities)):
        ...
        for q, topk_labels in tqdm.tqdm(zip(query_labels, topk_labels_list)):
            ...
            for r in topk_labels:
                ...
                total_q_prec += prec
                total_q_rec += rec
                total_q_f1 += (2 * prec * rec) / (prec + rec) if num_correct else 0.0
...
if __name__ == "__main__":
    ...
    main(args.ckpt_path, args.device)
